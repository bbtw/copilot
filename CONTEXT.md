...
- **LLM accessed through `services.llm`**. The module defines a provider-neutral prompt-completion protocol and an Anthropic adapter. Default: `claude-haiku-4-5`. Swappable without touching pipeline nodes.
- **ExplanationService uses dependency injection for LLM access**. It should depend on a small protocol/interface for prompt completion rather than importing a provider call directly, so production clients and test fakes can satisfy the same contract.
